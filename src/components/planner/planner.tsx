...
import { getLessonsForPeriod, generateBoardForLesson } from '@/lib/api'
...
export function Planner({ 
  ...
  // Auto-generate boards for lessons starting within 5 minutes
  const autoGeneratedBoards = useRef(new Set<string>())
  const checkAutoGenerate = useCallback(async () => {
    if (user?.role !== 'teacher') return
    const now = Date.now()
    for (const lesson of lessons) {
      if (lesson.boardId || lesson.board) continue
      if (autoGeneratedBoards.current.has(lesson.id)) continue
      ...
      if (minutesBefore <= 5 && minutesBefore > -120) {
        autoGeneratedBoards.current.add(lesson.id)
        try {
          await generateBoardForLesson(lesson.id)
  ...
  }, [lessons, user?.role, queryClient])

  useEffect(() => {
    checkAutoGenerate()
    const interval = setInterval(checkAutoGenerate, 60_000) // check every minute
